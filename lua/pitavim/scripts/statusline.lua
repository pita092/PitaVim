...
function statusline.fname()
	...
	if vim.bo.bt == "terminal" then
		...
		return id and cmd and string.format("[Terminal] %s (%s)", utils.stl.escape(cmd), id) or "[Terminal] %F"
	end

	-- Other special buffer types
	local prefix, suffix = bname:match("^%s*(%S+)://(.*)")
	if prefix and suffix then
		return string.format("[%s] %s", utils.stl.escape(utils.string.snake_to_camel(prefix)), utils.stl.escape(suffix))
	end
