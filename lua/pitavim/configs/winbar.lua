	...
	fzf = {
		...
		win_configs = {
			...
			width = function(menu)
				...
				local menu_width = menu._win_configs.width + border_width(menu._win_configs.border)
				local self_width = menu._win_configs.width
				local self_border = border_width(
					(M.opts.fzf.win_configs and M.eval(M.opts.fzf.win_configs.border, menu))
						or (menu.fzf_win_configs and M.eval(menu.fzf_win_configs.border, menu))
			...
			row = function(menu)
				...
				if type(menu_border) == "string" and menu_border ~= "shadow" and menu_border ~= "none" then
			...
			col = function(menu)
				...
				if type(menu_border) == "table" and menu_border[#menu_border] ~= "" then
					return -1
				end
				return 0
			end,
